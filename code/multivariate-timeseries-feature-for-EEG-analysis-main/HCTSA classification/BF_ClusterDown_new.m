...
if any(size(distMat)==1)
    ...
    distMat1 = distMat - diag(diag(distMat));
    distVec = squareform(distMat1); % convert for vector version
end
...
distMat = distMat - diag(diag(distMat)) + diag(Dij);
...
case 'abscorr'
    distMat0 = squareform(distVec0);
    distMat0 = distMat0 - diag(diag(distMat0)) + diag(diag(Dij));
