function [results,dscrp,idx] = find_Phase_High_Order(y,ch_labels,split_length,nplets,results,dscrp)
...
for i = 1:1:ava_ch_num
    sig = [sig angle(hilbert(signal{i}))];
end

% feature description
dscrp{end+1} = 'mean_Phase_O_Information';
dscrp{end+1} = 'mean_Phase_S_Information';
% dscrp{end+1} = 'mean_Redundancy';
dscrp{end+1} = 'mean_Phase_Synergy';
