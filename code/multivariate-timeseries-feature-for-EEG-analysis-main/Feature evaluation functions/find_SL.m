function [results, dscrp, idx] = find_SL(y,channel_labels, split_len,results, dscrp)
...
signal_segments = split_signal(signal, split_len);
...
for i = 1:CH
    for j = i:CH
        if i ~= j
            dscrp{end+1} = [EEG_chs{i}, '_vs_', EEG_chs{j}, '_synchronization'];
        end
    end
end
