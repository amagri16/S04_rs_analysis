function [results,dscrp,idx] = find_Phase_PTE(y,ch_labels,split_length,results,dscrp)
...
complex_s1 = hilbert(sig1);
sig1 = angle(complex_s1);

complex_s2 = hilbert(sig2);
sig2 = angle(complex_s2);

complex_s3 = hilbert(sig3);
sig3 = angle(complex_s3);
...
% feature description
dscrp{end+1} = string(sig1_ch)+'_'+string(sig3_ch)+'_to_'+string(sig2_ch)+'_Phase_Partial_Transfer_Entropy';
dscrp{end+1} = string(sig2_ch)+'_'+string(sig3_ch)+'_to_'+string(sig1_ch)+'_Phase_Partial_Transfer_Entropy';
dscrp{end+1} = string(sig1_ch)+'_'+string(sig2_ch)+'_to_'+string(sig3_ch)+'_Phase_Partial_Transfer_Entropy';
