function [results,dscrp,idx] = find_TE_BP(y,ch_labels,split_length,results,dscrp,filters)
    ...
    for n = (m+1:1:ava_ch_num)
        ...
        for mm = 1:size(bands,2) % band-wise TE
            for nn = 1:size(bands,2)               
                ...
                dscrp{end+1} = string(sig1_ch)+'_to_'+string(sig2_ch)+'BP_Transfer_Entropy'+'_'+bands(mm)+'_'+bands(nn);
                dscrp{end+1} = string(sig2_ch)+'_to_'+string(sig1_ch)+'BP_Transfer_Entropy'+'_'+bands(nn)+'_'+bands(mm);
