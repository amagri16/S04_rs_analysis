function [results,dscrp,idx] = find_phase_TE(y,ch_labels,split_length,results,dscrp)
...
for m = (1:ava_ch_num-1)
    for n = (m+1:1:ava_ch_num)
        ...
        % Hilbert transform to get phase angle
        ...
        % feature description
        dscrp{end+1} = string(sig1_ch)+'_to_'+string(sig2_ch)+'_Phase_Transfer_Entropy';
        dscrp{end+1} = string(sig2_ch)+'_to_'+string(sig1_ch)+'_Phase_Transfer_Entropy';
