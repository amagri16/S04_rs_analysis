function [results,dscrp,idx] = find_wavelet_TE(y,f_s,ch_labels,split_length,results,dscrp)
...
for m = (1:ava_ch_num-1)
    ...
    %for n = ava_ch_num
        ...
        % sig1 = sig1(2:end) - sig1(1:end-1);
        % sig2 = sig2(2:end) - sig2(1:end-1);

        sig1_filt = [];
        sig2_filt = [];

        s1_f1 = icwt(cwt(sig1,'morse', f_s,'FrequencyLimits',[1 8],'VoicesPerOctave',20));
        s2_f1 = icwt(cwt(sig2,'morse', f_s,'FrequencyLimits',[1 8],'VoicesPerOctave',20));
        s1_f2 = icwt(cwt(sig1,'morse', f_s,'FrequencyLimits',[8 30],'VoicesPerOctave',20));
        s2_f2 = icwt(cwt(sig2,'morse', f_s,'FrequencyLimits',[8 30],'VoicesPerOctave',20));      
        ...
        for mm = 1:size(bands,2) % band-wise TE
            for nn = 1:size(bands,2)
                ...
                if n ~= ava_ch_num
                    dscrp{end+1} = string(sig1_ch)+'_to_'+string(sig2_ch)+'_Wavelet_Transfer_Entropy'+'_'+bands(mm)+'_'+bands(nn);
                    dscrp{end+1} = string(sig2_ch)+'_to_'+string(sig1_ch)+'_Wavelet_Transfer_Entropy'+'_'+bands(nn)+'_'+bands(mm);
                else
                    dscrp{end+1} = string(sig1_ch)+'_to_'+string(sig2_ch)+'_Wavelet_Transfer_Entropy'+'_'+bands(mm);
                    dscrp{end+1} = string(sig2_ch)+'_to_'+string(sig1_ch)+'_Wavelet_Transfer_Entropy'+'_'+bands(mm);
                end
