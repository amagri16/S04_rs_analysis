function [results,dscrp,idx] = find_BP_TE_within_ch(y,ch_labels,split_length,results,dscrp,filters)
...
for m = 1:ava_ch_num
    ...
    for mm = 1:size(bands,2) % band-wise TE
        for nn = 1:size(bands,2)
            ...
            if mm~=nn
                dscrp{end+1} = string(sig1_ch)+'_BP_Transfer_Entropy'+'_'+bands(mm)+'_'+bands(nn);
