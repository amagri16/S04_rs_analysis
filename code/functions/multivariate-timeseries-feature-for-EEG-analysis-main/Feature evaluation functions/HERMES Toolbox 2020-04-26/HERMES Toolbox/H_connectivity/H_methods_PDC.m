...
% if PDC_on, output.PDC.data = zeros ( channels, channels, config.nfft, windows ); end % FIXED 2019-11-30
% if DTF_on, output.PDC.data = zeros ( channels, channels, config.nfft, windows ); end % FIXED 2019-11-30
if PDC_on, output.PDC.data = zeros ( channels, channels, ceil ( ( config.nfft + 1 ) / 2 ), windows ); end
if DTF_on, output.PDC.data = zeros ( channels, channels, ceil ( ( config.nfft + 1 ) / 2 ), windows ); end
...
%     [ w A ] = arfit ( squeeze ( data ( :, :, :, window ) ), config.orderMAR, config.orderMAR );
    ...
    A = B;
%     Af = fft ( A, 2 * config.nfft, 3 ); % FIXED 2019-11-30
%     Af = Af ( :, :, 1: config.nfft ); % FIXED 2019-11-30
    Af = fft ( A, config.nfft, 3 );
    Af = Af ( :, :, 1: ceil ( ( config.nfft + 1 ) / 2 ) );
...
%     for f = 1: config.nfft % FIXED 2019-11-30
    for f = 1: ceil ( ( config.nfft + 1 ) / 2 )
