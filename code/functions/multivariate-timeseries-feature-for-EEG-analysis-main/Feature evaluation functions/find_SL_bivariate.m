function [results,dscrp,idx] = find_SL_bivariate(y,ch_labels,split_length,results,dscrp)
...
for m = (1:ava_ch_num-1)
    ...
    %for n = ava_ch_num
        ...
        % feature description
        dscrp{end+1} = string(sig1_ch)+'_vs_'+string(sig2_ch)+'_Synchronization_Likelihood';
        dscrp{end+1} = string(sig1_ch)+'_vs_'+string(sig2_ch)+'_Overall_Synchronization';
        ...
        for k = 1:1:min_sig_size
            ...
            sig_all = [sig1_now sig2_now];
            [S_matrix, S] = synchronization_likelyhood(sig_all);
            SL_all = [SL_all S_matrix(1,2)];
            S_all = [S_all S];

        end
        results = [results mean(SL_all)];
