...
for m = 1:ava_ch_num
    ...
    y_MA = y;
    clear y
    y = y_MA; % do not undesrand why we take a differene between EEG time points and EG time points 1 data point sage
    ...
    results = [results log(delta_power)];
    results = [results log(theta_power)];
    results = [results log(alpha_power)];
    results = [results log(beta_power)];
    results = [results log(beta_narrow_power)];
    results = [results log(gamma_power)];

    results = [results log(delta_power/theta_power)];
    results = [results log(delta_power/alpha_power)];
    results = [results log(delta_power/beta_power)];
    results = [results log(theta_power/alpha_power)];
    results = [results log(theta_power/beta_power)];
    results = [results log(alpha_power/beta_power)];

    results = [results log((delta_power+theta_power)/(alpha_power+beta_power))];    % Transition power ratio

    ft_dscrp_y = [ft_dscrp_y,{'delta','theta','alpha','beta','beta_25_29','gamma','DTratio','DAratio','DBratio','TAratio','TBratio','ABratio','Transratio'}];

    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % Peak Oscillation frequency
    f_delta = f(delta_idx);
    results = [results f_delta(find(psd_y_norm(delta_idx) == max(psd_y_norm(delta_idx)),1))];

    f_theta = f(theta_idx);
    results = [results f_theta(find(psd_y_norm(theta_idx) == max(psd_y_norm(theta_idx)),1))];

    f_alpha = f(alpha_idx);
    results = [results f_alpha(find(psd_y_norm(alpha_idx) == max(psd_y_norm(alpha_idx)),1))];

    f_beta = f(beta_idx);
    results = [results f_beta(find(psd_y_norm(beta_idx) == max(psd_y_norm(beta_idx)),1))];

    f_beta_narrow = f(beta_narrow_idx);
    results = [results f_beta_narrow(find(psd_y_norm(beta_narrow_idx) == max(psd_y_norm(beta_narrow_idx)),1))];

    f_gamma = f(gamma_idx);
    results = [results f_gamma(find(psd_y_norm(gamma_idx) == max(psd_y_norm(gamma_idx)),1))];

    ft_dscrp_y = [ft_dscrp_y,{'deltapeak','thetapeak','alphapeak','betapeak','beta_25_29_peak','gammapeak'}];

    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % Temporal coherence features

    epc_len = 1;     % epoch length for feature extraction in 's'
    ...
    results = [results mean(Cxy_y_avg(delta_idx), 'omitnan')];
    results = [results mean(Cxy_y_avg(theta_idx), 'omitnan')];
    results = [results mean(Cxy_y_avg(alpha_idx), 'omitnan')];
    results = [results mean(Cxy_y_avg(beta_idx), 'omitnan')];
    ...
    results = [results abs(sum(zt_DT)/sum(amp_theta(1:idx_cycle_delta)))];
    results = [results abs(sum(zt_DA)/sum(amp_alpha(1:idx_cycle_delta)))];
    results = [results abs(sum(zt_DB)/sum(amp_beta(1:idx_cycle_delta)))];
    results = [results abs(sum(zt_TA)/sum(amp_alpha(1:idx_cycle_theta)))];
    results = [results abs(sum(zt_TB)/sum(amp_beta(1:idx_cycle_theta)))];
    results = [results abs(sum(zt_AB)/sum(amp_beta(1:idx_cycle_alpha)))];
