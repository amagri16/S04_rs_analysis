function [results,dscrp,idx] = find_wavelet_TE_within_ch(y,f_s,ch_labels,split_length,results,dscrp)
...
%bands = ["delta","theta","alpha","beta","beta1","beta2","beta3","low_band","gamma"];
...
for m = 1:ava_ch_num
    ...
    for mm = 1:size(bands,2) % band-wise TE
        for nn = 1:size(bands,2)
            ...
            if mm~=nn
                dscrp{end+1} = string(sig1_ch)+'_Wavelet_Transfer_Entropy'+'_'+bands(mm)+'_'+bands(nn);
                ...
                for i=0:1:num_sig1-1
                    sig2_all(:,i+1)=sig1_filt(i*split_len+1:1:i*split_len+(split_len),nn);
                end
