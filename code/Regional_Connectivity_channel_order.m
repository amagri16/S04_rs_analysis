...
ft_all_single_names = fieldnames(ft_all_single);
ft_all_undirected_names = fieldnames(ft_all_undirected);
ft_all_directed_names = fieldnames(ft_all_directed);

% single channel features
ft_single = [];
dscrp_single = [];
for i = 1:size(ft_all_single_names,1)
    ft_name = ft_all_single_names{i};
    ft_val = ft_all_single.(ft_name);
    if ~isempty(ft_val)
        ft_single = [ft_single, mean(ft_val,2)];
        dscrp_single = [dscrp_single; {ft_name}];
    end
end
...
ft_undirected = [];
dscrp_undirected = [];
for i = 1:size(ft_all_undirected_names,1)
    ft_name = ft_all_undirected_names{i};
    ft_val = ft_all_undirected.(ft_name);
    if ~isempty(ft_val)
        ft_undirected = [ft_undirected, mean(ft_val,2)];
        dscrp_undirected = [dscrp_undirected; {ft_name}];
    end
end
...
ft_directed = [];
dscrp_directed = [];
for i = 1:size(ft_all_directed_names,1)
    ft_name = ft_all_directed_names{i};
    ft_val = ft_all_directed.(ft_name);
    if ~isempty(ft_val)
        ft_directed = [ft_directed, mean(ft_val,2)];
        dscrp_directed = [dscrp_directed; {ft_name}];
    end
end

% concatonate all features
features = [ft_single, ft_undirected, ft_directed];
dscrp = [dscrp_single; dscrp_undirected; dscrp_directed];
